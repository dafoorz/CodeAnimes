...
export default function QuizOnlineConnect() {
  const hostRoom = useQuizNetStore((s) => s.hostRoom);
  const joinRoom = useQuizNetStore((s) => s.joinRoom);
  const connecting = useQuizNetStore((s) => s.connecting);
  const error = useQuizNetStore((s) => s.error);
  const clearError = useQuizNetStore((s) => s.clearError);
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-md flex-col justify-center gap-6 px-6 py-12">
      ...
      <div className="text-center">
        <h1 className="font-serif text-3xl font-bold text-white">Opening Quiz · Online</h1>
        <p className="mt-2 text-sm text-white/60">
          Race your friends to name each opening. Create a room and share the
          code, or join one.
        </p>
      </div>
      ...
      {mode === 'choose' ? (
        <div className="flex flex-col gap-3">
          ...
          {!trimmed && (
            <p className="text-center text-xs text-white/40">Enter a name to continue.</p>
          )}
        </div>
      ) : (
        <div className="flex flex-col gap-3">
