...
export default function GAnimeConnect() {
  const hostRoom = useGAnimeNetStore((s) => s.hostRoom);
  const joinRoom = useGAnimeNetStore((s) => s.joinRoom);
  const connecting = useGAnimeNetStore((s) => s.connecting);
  const error = useGAnimeNetStore((s) => s.error);
  const clearError = useGAnimeNetStore((s) => s.clearError);
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-md flex-col justify-center gap-6 px-6 py-12">
      ...
      <div className="text-center">
        <h1 className="font-serif text-3xl font-bold text-white">Guess the Anime · Online</h1>
